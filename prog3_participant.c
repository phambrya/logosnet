...
Project 3: LogosNet
Due: July 31, 2021
Description: This is a participant file that will ask for the user name so it could 
start on send messages to the server. The participant's primary job is to send messages.
*/
			...
			if(FD_ISSET(i, &read_fds)){
				if(hasUsrName == 1) {
					...
					if (strcmp(send_buf, "/quit") == 0) {
						...
						//send the size
						uint16_t msgSizeOut = strlen(send_buf);
						uint8_t check16 = 16;
						send(sd, &check16, 1, 0);
						send(sd, &msgSizeOut, sizeof(uint16_t), 0);
						//take off the null term
						send(sd, send_buf, msgSizeOut, 0);
						memset(&send_buf[0], 0, sizeof(send_buf));
						i++;
					}
				}
				if(i == STDIN_FILENO){ //if console input
					send_buf[strcspn(send_buf, "\n")] = 0;
					if (strcmp(send_buf, "/quit") == 0) {
						close(sd);
						exit(1);
				...
				{ //if recv input
					...
					recv(i, recv_buf, sizeof(recv_buf), 0);
					if(hasUsrName == 0){
						if(connected == 0){ //add client to server 
							if(recv_buf[0] == 'Y'){
								//connect client
								connected = 1;
								uint16_t clientType = 1; // 1 if participant 
								int user_tf = 0;
								uint8_t msgSizeOut;
								char temp_buf[15];
								char name_buf[10];
